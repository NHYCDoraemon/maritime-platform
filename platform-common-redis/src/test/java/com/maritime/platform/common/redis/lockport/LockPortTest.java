...
class LockPortTest {
    ...
    void tryLock_withUnclaimedKey_returnsHandle() {
        Optional<LockHandle> handle = lockPort.tryLock(
                "resource", "R1", Duration.ofMillis(100), Duration.ofSeconds(10));

        assertThat(handle).isPresent();
        assertThat(handle.get().lockKey()).isEqualTo("platform:lock:resource:R1");
        handle.get().close();
    }
    ...
    void tryLock_whenAlreadyLocked_returnsEmpty() {
        Optional<LockHandle> first = lockPort.tryLock(
                "resource", "R2", Duration.ofMillis(100), Duration.ofSeconds(10));
    ...
    void tryLock_afterUnlock_succeeds() {
        Optional<LockHandle> first = lockPort.tryLock(
                "resource", "R3", Duration.ofMillis(100), Duration.ofSeconds(10));
        assertThat(first).isPresent();
        first.get().unlock();
    ...
    void isLocked_reflectsState() {
        assertThat(lockPort.isLocked("resource", "R4")).isFalse();

        Optional<LockHandle> handle = lockPort.tryLock(
                "resource", "R4", Duration.ofMillis(100), Duration.ofSeconds(10));
        assertThat(handle).isPresent();

        assertThat(lockPort.isLocked("resource", "R4")).isTrue();
    ...
    void close_releasesLock() {
        Optional<LockHandle> first = lockPort.tryLock(
                "resource", "R5", Duration.ofMillis(100), Duration.ofSeconds(10));
        assertThat(first).isPresent();

        try (LockHandle ignored = first.get()) {
            assertThat(lockPort.isLocked("resource", "R5")).isTrue();
        }

        assertThat(lockPort.isLocked("resource", "R5")).isFalse();
    }

    @Test
    void unlock_isIdempotent() {
        Optional<LockHandle> handle = lockPort.tryLock(
                "resource", "R6", Duration.ofMillis(100), Duration.ofSeconds(10));
        assertThat(handle).isPresent();
    ...
    void unlock_doesNotReleaseAnotherTokensLock() {
        Optional<LockHandle> owner = lockPort.tryLock(
                "resource", "R7", Duration.ofMillis(100), Duration.ofSeconds(10));
        ...
        owner.get().close();
        redisTemplate.opsForValue().set("platform:lock:resource:R7", "OTHER_TOKEN");
